...
async function addFeature() {
    ...
    while (userInputNumber != -1) {
        userInputNumber = await inquirer.prompt([
            {
                type: "number",
                name: "userInputNumber",
                message: "Enter 1 to add feature, -1 to exit"
            }
        ]);
        if (userInputNumber == 1) {
            let userFeatureInput = await inquirer.prompt([
                {
                    type: "string",
                    name: "userFeatureInput",
                    message: "Enter a feature"
                }
            ]);
            if (userFeatureInput.userFeatureInput) {
                packages[0].feature.push(userFeatureInput.userFeatureInput);
            }
        }
    }
}
...
function showMeAllPackages(showFeature) {
